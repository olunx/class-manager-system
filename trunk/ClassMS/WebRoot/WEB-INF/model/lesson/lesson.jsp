			...
			<form action="deleteManyLesson" method="post">
				...
				<input type="submit" value="确定" />
				 ...
				 <s:if test="%{pageBean.currentPage == 1}">  
					 ...
					 <s:else>  
					     <a href="<%=path %>/lesson/listLesson?page=1">第一页</a>  
					     <a href="<%=path %>/lesson/listLesson?page=<s:property value="%{pageBean.currentPage-1}"/>">上一页</a>  
					 </s:else>  
					 <s:if test="%{pageBean.currentPage != pageBean.totalPage}">  
					     <a href="<%=path %>/lesson/listLesson?page=<s:property value="%{pageBean.currentPage+1}"/>">下一页</a>  
					     <a href="<%=path %>/lesson/listLesson?page=<s:property value="pageBean.totalPage"/>">最后一页</a>  
					 </s:if>  
					 <s:else>  
					     下一页 最后一页  
					 </s:else>  
